...
export class SQSPoller {
  ...
  async startPolling() {
    ...
    if (!this.queueUrl) {
      console.warn(
        "MC_TO_WEB_QUEUE_URL is not configured. SQS polling disabled.",
      );
      return;
    }
  ...
  private async processMessage(message: {
    ...
    try {
      ...
      const messageData: SQSAuthTokenMessage | SQSOtpResponseMessage =
      ...
      if (messageData.type === "auth_token") {
        ...
        await this.handleOtpResponseMessage(
          messageData as SQSOtpResponseMessage,
        );
        ...
        console.warn(
          ...
          (messageData as { type: string }).type,
        );
      }
    } catch (error) {
