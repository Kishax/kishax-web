...
export class SqsMessageProcessor {
  ...
  private messageHandlers: Map<string, (message: SqsMessage) => Promise<void>> = new Map()
   ...
   * メッセージタイプ別のハンドラーを登録
   */
  registerHandler(messageType: string, handler: (message: SqsMessage) => Promise<void>) {
  ...
  private async processMessage(message: Message) {
    ...
    try {
      ...
      const source = message.MessageAttributes?.source?.StringValue || "unknown"
      const timestamp = message.MessageAttributes?.timestamp?.StringValue || new Date().toISOString()
...
export function getSqsMessageProcessor(config?: Partial<SqsConfig>): SqsMessageProcessor {
  if (!sqsProcessor) {
