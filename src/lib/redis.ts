 ...
 * OTPレスポンスをRedisに保存
 */
export async function setOtpResponse(
  mcid: string,
  uuid: string,
  response: { success: boolean; message: string; timestamp: number },
) {
 ...
 * OTPレスポンスをRedisから取得
 */
export async function getOtpResponse(
  mcid: string,
  uuid: string,
): Promise<{
  success: boolean;
 ...
 * OTPレスポンスが存在するかチェック
 */
export async function hasOtpResponse(
  mcid: string,
  uuid: string,
): Promise<boolean> {
  try {
 ...
 */
export async function waitForOtpResponse(
  mcid: string,
  uuid: string,
  timeoutMs: number = 30000,
): Promise<{ success: boolean; message: string; timestamp: number } | null> {
