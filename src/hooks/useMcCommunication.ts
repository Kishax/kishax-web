...
export function useMcCommunication() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('mcAuthResponse', handleAuthResponse as EventListener)
    window.addEventListener('mcPlayerStatus', handlePlayerStatus as EventListener)
    window.addEventListener('mcServerInfo', handleServerInfo as EventListener)

    return () => {
      window.removeEventListener('mcAuthResponse', handleAuthResponse as EventListener)
      window.removeEventListener('mcPlayerStatus', handlePlayerStatus as EventListener)
      window.removeEventListener('mcServerInfo', handleServerInfo as EventListener)
    }
  }, [])

  // MC認証確認送信
  const sendAuthConfirm = useCallback(async (playerName: string, playerUuid: string) => {
    try {
  ...
  // テレポートコマンド送信
  const sendTeleportCommand = useCallback(async (playerName: string, location: string) => {
    try {
      const result = await apiClient.sendTeleportCommand(playerName, location)
      return result
    } catch (error) {
  ...
  // サーバー切り替えコマンド送信
  const sendServerSwitchCommand = useCallback(async (playerName: string, serverName: string) => {
    try {
      const result = await apiClient.sendServerSwitchCommand(playerName, serverName)
      return result
    } catch (error) {
  ...
  // メッセージ送信
  const sendMessage = useCallback(async (playerName: string, message: string) => {
    try {
  ...
  // サーバーステータスリクエスト
  const requestServerStatus = useCallback(async (playerName: string, serverName?: string) => {
    try {
      const result = await apiClient.requestServerStatus(playerName, serverName)
      return result
    } catch (error) {
  ...
  // プレイヤーリストリクエスト
  const requestPlayerList = useCallback(async (playerName: string, serverName?: string) => {
    try {
      const result = await apiClient.requestPlayerList(playerName, serverName)
      return result
    } catch (error) {
  ...
  // 特定プレイヤーの最新認証レスポンス取得
  const getLatestAuthResponse = useCallback((playerUuid: string) => {
    return authResponses
      .filter(response => response.playerUuid === playerUuid)
      .sort((a, b) => new Date(b.message).getTime() - new Date(a.message).getTime())[0]
  }, [authResponses])

  // 特定プレイヤーのステータス取得
  const getPlayerStatus = useCallback((playerUuid: string) => {
